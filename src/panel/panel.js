...
$("#save").addEventListener("click", async () => { try { busy(true); const response=await send({type:"workspace:save",draft:state.draft}); state=createEditorState(response.snapshot); render(); toast("已保存到 Chrome 原生标签组"); } catch(error){toast(error.message,true)} finally{busy(false)} });
